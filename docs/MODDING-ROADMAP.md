...
- **状态**: `[~]` 进行中(2026-07-09)— m16a/b/c `[x]` 合 master(636 tests 绿);m16d android smoke `[~]` 并行(本批,`docs/SMOKE-M16.md`,状态视回报收口)
...
- **范围**(3 + 1 feature,m16a/b/c 合 master,m16d smoke 并行):
  ...
  - **m16d android smoke**:`docs/SMOKE-M16.md` android 真机冒烟(m16a/b/c release 端到端);`[~]` 并行,状态视回报收口
- **验收**: m16a/b/c 合 master(636 tests 绿);runtime 自定义贴图可用;remixed_full 开箱可玩 alpha;mod diagnostics UI 可定位加载错误;m16d smoke 视回报
...
| M16 可玩内容包 alpha | `[~]` | M15 | 中 | 4(m16a/b/c + m16d smoke) | runtime 自定义 spriteFile(独立 PNG,`78174c2ed`)+ remixed_full minimal playable alpha(10 item/5 spell/6 mob/1 shop/1 hub,default_enabled=true,`3d36f4cfe`)+ mod diagnostics UI(ModDiagnostics+WndModDetails+status,`16baac444`);m16a/b/c 合 master(636 tests),m16d android smoke `[~]` 并行 |
| **合计(M0-M16)** | | | | **64 完成 / 65 计划** | M0-M15 全完成(61)+ M16 m16a/b/c 完成、m16d smoke 进行中;fork 从 modding beta → 可玩内容包 alpha;主游戏池注入 Lua 内容 + runtime 自定义贴图 + 开箱可玩 remixed_full + mod diagnostics UI |
