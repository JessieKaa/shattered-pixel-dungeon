...
| M13 玩家 UX 补完 + 内置内容包 | `[x]` | M12 | 中 | 3(M13a/b/c) | mod 卸载/更新 UI(ModInstaller.removeMod origin 守卫 + 长按卸载)+ 自定义关卡发现 UI(enterLevel 门放开 registered + WndCustomLevels + WndGame 入口)+ 内置内容包 remished_lite(default_enabled=true showcase hub,main game 原版 C3 纯净);598 tests |
| **合计(M0-M13 全完成)** | | | | **54 完成** | M0-M13(54)全闭合;fork 可交付 + modding 平台广度 + 移植内容最小可玩 + mod 分发与生态闭环 + 玩家 UX 补完(卸载/关卡发现)+ 开箱内容展示 |
