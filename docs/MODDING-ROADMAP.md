...
### M13 玩家 UX 补完 + 内置内容包(2026-07-09 立项并完成)

- **状态**: `[x]` 完成(2026-07-09)— 3 feature 合 master(`1c00cda0e`),598 tests 绿;mod 系统端到端可用(卸载/更新 + 关卡发现 + 开箱内容)
- **目标**:补 M12 import 的逆向缺口(卸载/更新)+ 让注册关卡玩家可进 + 开箱即有一个可玩内容展示
- **动机**:M12 后玩家能导 mod 但删不掉;导入/注册的关卡 debug-gated 玩家进不去;3 个 mod 全 default_enabled=false,无开箱内容。M13 补玩家侧真实缺口
- **范围**(3 feature,全并行,文件域两两不相交):
  - **M13a mod 卸载/更新 UI**:`ModInstaller.removeMod(id, cb)`(origin 守卫只删 EXTERNAL,builtin 拒)+ WndModManager 长按外部 mod → `WndOptions` 确认 → 删 `mods_user/<id>/` + 提示重启;import `already_exists` 文案指向卸载入口(更新 = 卸载旧 + 重导,Option C 约束内可行)(`a1f5cdbba`)
  - **M13b 自定义关卡发现 UI**:`enterLevel` 门从 `!isDebug()` 放开为 `!isEnterAllowed(id)`(registered 关卡 release 可进,未注册仍 debug-only,镜像 `injectLevelTraps` 先例)+ `listEnterableLevels()` + `WndCustomLevels` 列表窗口 + `WndGame`「自定义关卡」按钮(有关卡才显示)(`6f777e7b6`)
  - **M13c 内置内容包 remished_lite**:`default_enabled=true` 的 builtin mod,一个 32×32 showcase hub 关卡(guide NPC 发武器 + sage NPC lore + shop 卖 vanilla 消耗品);内容只在关卡内(**main game 原版不变,C3 纯净** —— 不 register_trap、不改 Generator.LUA_ITEM prob、spawn 池不读 LuaMobRegistry);spell/mob 因无放置/获取路径 registered-but-inert(覆盖度 + 诚实标注)(`bea5915e9`)
- **验收**:598 tests 全绿;mod 可卸载/更新;注册关卡玩家可进;remished_lite 开箱提供一个可玩 showcase;C3 全程守住(main game 原版)
- **依赖**:M12(已完成)
- **风险**:
  - 卸载删文件安全 —— origin 守卫(只 EXTERNAL)+ `baseDir.deleteDirectory()` 限定子树 + 确认框防误删
  - `default_enabled=true` 改变"默认关闭"路线(§1.4)—— 内容只在 showcase 关卡内(main game 原版),C3 守住;测试侧 `ModTestSupport.enableTestMod` 中心禁用防污染断言
  - enterLevel 门放开 —— 只 registered 放行(未注册裸 asset 仍锁),C3 守
  - codex 评审 infra 持续 503 —— 政策 B(:core:test 硬验收 + C3 自审 + dispatcher 抽检)
- **难度**:中(M13a/b 是 UI + 小 API;M13c 是 asset 创作 + C3 守护)
- **预估 feature 数**:3(M13a/b/c)

### M14 发布硬化(2026-07-09 立项并完成,v3.4.0 cut)

- **状态**: `[x]` 完成(2026-07-09)— 2 feature 合 master(`79e9dfa04`),599 tests 绿;**v3.4.0 cut**(versionCode 896→900 + tag);fork 发布基线确立
- **目标**:为 v3.4.0 发布(default-enabled remished_lite)守质量 + 打包:C3 回归硬化 + showcase 平衡 + R8 keep 审计 + versionCode bump + 签名 release APK
- **动机**:M13 后 fork 功能完整(54 feature),但未做发布前硬化(C3 全量断言 / 平衡精调 / R8 release 验证 / versionCode bump);M14 补齐发布门槛,cut v3.4.0
- **范围**(2 feature,并行,文件域不相交):
  - **M14a R8 keep 审计 + v3.4.0 versionCode bump + release 打包**:`build.gradle` `appVersionCode 896→900` / `appVersionName '3.3.8'→'3.4.0'`;R8 keep 审计(宽 `-keepnames com.shatteredpixel.**` + `-keep modding.**` + luaj keep 已覆盖 M12/M13,**零 keep 改动**);签名 release APK 构建(28.9MB,apksigner 过,test keystore);install+launch 不崩(EGL 黑屏经 debug 对照证为设备 quirk 非 release 回归)(`feab779ea`)
  - **M14b C3 回归硬化 + showcase 平衡**:扩展 `RemishedLitePackTest`(反射 `Generator.categoryProbs` 证 LUA_ITEM built-deck 权重 0 + 驱动 `MobSpawner.getMobRotation` depth 1..26 证无 LuaMob —— 真运行时行为断言,非静态/grep);showcase 平衡(showcase-local,不动全局 BalanceConfig):lantern_blade `attackProc +2→+1`(Shortsword tier2 parity)+ shop 统一 2× vanilla `value()` + finite quantity(消 inconsistency + 防无限库存)+ guide NPC one-shot 守卫(防 spam 刷分)(`ff09e3c13`)
- **验收**:599 tests 全绿;签名 release APK 构建过 + launch 不崩;R8 零 keep 回归;C3 真行为断言(drop deck 权重 + spawn rotation);showcase 平衡对标 vanilla;versionCode 900/3.4.0;v3.4.0 tag
- **依赖**:M13(已完成)
- **风险**:
  - R8 minify 破新反射点(C5)—— 宽 keep 覆盖 + release smoke 验证(零回归)
  - default-on 平衡(C3)—— 保守:只调 showcase-local,不动全局 BalanceConfig(M9c 已调);C3 真行为断言守
  - device EGL 坏 —— smoke 降级(launch-不崩 + debug 对照),不阻塞发布
  - codex infra 503 —— 政策 B(:core:test + assembleRelease 硬验收)
- **难度**:中(M14a 是构建验证 + version bump;M14b 是测试扩展 + showcase 数值精调)
- **预估 feature 数**:2(M14a/b)
- **v3.4.0 发布动作**:versionCode bump + git tag v3.4.0(supervisor 合并后统一打)+ 签名 release APK。release 用 test keystore(正式签名留后续)

### M15 main-game 集成(Lua 内容进主游戏池,2026-07-09 立项并完成)

- **状态**: `[x]` 完成(2026-07-09)— 5 feature 合 master;Lua item/mob/spell 从「registered-but-inert」(M13c 遗留)推进到进主游戏掉落/刷怪/获取池,shop 卖 Lua 内容;C3 守住(mod 禁用 = 零 Lua 内容进主游戏,真行为断言延续 M14b)
- **目标**: 把 M10/M13 注册但 inert 的 Lua item/mob/spell 接进主游戏生成与经济链路,让 default-enabled 内容包在主游戏一周目真实出现
- **动机**: M13c 起内容包 default_enabled=true 但 item/mob/spell registered-but-inert(无放置/获取路径),内容只活在 showcase 关卡;M15 补掉落/刷怪/获取池/商店四条主游戏注入路径 + 导入 remixed 美术资产为 M16 打底
- **范围**(5 feature,全并行,文件域不相交):
  - **m15a Lua item 主游戏掉落池**:`Generator.setLuaItemProbability` + `BalanceConfig.lua_item_drop_prob`(max 聚合)→ LUA_ITEM 类别按 prob 进掉落 deck;mod 禁用 prob=0(C3 守);`GeneratorLuaItemTest` 覆盖 deck/mod 禁用/fullReset/非法值(`db1d8b2ee`)
  - **m15b Lua mob 主游戏刷怪池**:`LuaMobFactory`(占位)+ `Level.createMob` factory 分支 + `MobSpawner` prob 替换 + `BalanceConfig.lua_mob_spawn_prob` → spawn rotation 按 prob 注入 LuaMob;`MobSpawnerLuaMobTest` zero-prob C3 baseline(`172e628dd`)
  - **m15c Lua spell 获取路径**:`Generator.Category.LUA_SPELL` + `LuaSpellPool` + `lua_spell_drop_prob` → spell 经掉落 deck 获取;`GeneratorLuaSpellTest` 覆盖 pool/deck/zero-prob/balance 传播(`cff423f64`)
  - **m15d Lua shop 卖任意 item/spell**:`LuaShopItems.create` 查找顺序 LuaItemRegistry→LuaSpellRegistry→vanilla whitelist;未知 id 保持 null + error log,无反射;`LuaShopNpc.attemptBuy` 对 LuaItem/LuaSpell 通用(均继承 Item);5 测试覆盖 item/material/spell + vanilla 优先级 + attemptBuy 端到端(实现 `c8a86b5c4`;PLAN 文档 commit `ce7cc5b72`)
  - **m15e remixed sprites 资产导入**:498 PNG(spritesheets/icons/effects)+ `docs/SPRITE-MAP.md` + `docs/MOD-SPRITES.md`,为 M16 runtime spriteFile 与 remixed_full 美术打底(`4c0607d5e`)
- **验收**: Lua item/mob/spell 按 prob 进主游戏生成池(mod 禁用 = 零注入,C3 守住);shop 卖 Lua 内容;remixed 美术资产就位;主游戏一周目带 Lua 内容可玩
- **依赖**: M14(已完成;在 v3.4.0 发布基线上接主游戏池)
- **风险**:
  - 改主游戏生成池有 C3 风险 —— `BalanceConfig.lua_*_prob` max 聚合 + mod 禁用 prob=0 守 C3(真行为断言,延续 M14b Generator drop-deck / MobSpawner rotation 模式)
  - 资产导入体量大(498 PNG)—— 仅 assets,零代码风险
- **难度**: 中(m15a/b/c 是 Generator/MobSpawner prob 注入 + C3 真行为断言;m15d 是 shop 查找链;m15e 纯资产)
- **预估 feature 数**: 5(m15a/b/c/d/e)

### M16 可玩内容包 alpha(remixed_full + runtime sprites + diagnostics,2026-07-09 立项)

- **状态**: `[x]` 完成(2026-07-10)— m16a/b/c `[x]` 合 master(636 tests 绿);m16d android smoke `[x]` 通过(`docs/SMOKE-M16.md`:assembleDebug 33.8MB + install Success + am start START_SUCCESS + 无 FATAL/ANR;UI 深度交互落为人工 checklist)
- **目标**: 从 modding beta(M0-M15 平台 + 移植)推进到「可玩内容包 alpha」—— runtime 自定义贴图 + 一个 minimal playable 内容包(remixed_full)+ mod diagnostics UI,让 fork 默认带一个真实可玩、自带贴图、可诊断的内容包
- **动机**: M15 让 Lua 内容进主游戏,但贴图仍靠内置 spriteMap;无「开箱即玩」的完整内容包(M13c remished_lite 只是 showcase hub);mod 出问题无诊断 UI。M16 把这三块补齐
- **范围**(3 + 1 feature,m16a/b/c 合 master,m16d smoke 并行):
  - **m16a runtime mod spriteFile**:Lua item/spell/mob 独立 PNG(`spriteFile` 字段 + LuaItem/LuaSpell/LuaMob spriteFile 解析 + ModMobSprite 选择);mod 自带 PNG 即用,不依赖内置 spriteMap;`LuaSpriteFileTest` 覆盖 item/spell/mob 解析 + ModMobSprite 选择;628 tests(`78174c2ed`)
  - **m16b remixed_full minimal playable alpha**:`remixed_full` builtin 内容包,default_enabled=true,10 item / 5 spell / 6 mob / 1 shop / 1 hub;一个最小可玩 alpha(复用 M15 主游戏池注入 + m16a runtime 贴图);`RemixedFullPackTest` 覆盖 enabled/disabled/registries/balance/hub;C3 守(mod 禁用零注入)(`3d36f4cfe`)
  - **m16c mod diagnostics UI**:`ModDiagnostics` + `WndModDetails`(展示 mod load/parse 错误)+ `WndModManager` status 状态点;mod 加载失败可定位;`ModDiagnosticsTest`(9)+ `ModScannerTest`/`LuaModEntryTest` 扩展;636 tests(`16baac444`)
  - **m16d android smoke**:`docs/SMOKE-M16.md` android 真机冒烟(m16a/b/c release 端到端);`[x]` 通过(assembleDebug 33.8MB + install Success + am start START_SUCCESS + 无 FATAL/ANR;UI 深度交互落为人工 checklist)
- **验收**: m16a/b/c 合 master(636 tests 绿);runtime 自定义贴图可用;remixed_full 开箱可玩 alpha;mod diagnostics UI 可定位加载错误;m16d smoke 通过(无 fatal crash)
- **依赖**: M15(已完成;主游戏池注入 + 资产就位)
- **风险**:
  - runtime spriteFile 解析(R8 keep + asset 路径穿越,m16a)—— keep 已覆盖 `modding.**`(M14a),路径校验复刻 M12 `ModInstaller`
  - remixed_full default_enabled=true 改变默认体验(C3)—— 内容进主游戏池但受 `BalanceConfig` prob 控制,M15 真行为断言延续
  - diagnostics UI 侵入 `WndModManager` —— 单点 status 点,不重构
- **难度**: 中(m16a runtime 贴图解析;m16b 内容包创作 + C3;m16c diagnostics UI;m16d smoke)
- **预估 feature 数**: 4(m16a/b/c + m16d smoke)
...
| M13 玩家 UX 补完 + 内置内容包 | `[x]` | M12 | 中 | 3(M13a/b/c) | mod 卸载/更新 UI + 自定义关卡发现 UI + 内置内容包 remished_lite(default_enabled=true showcase,main game 原版 C3 纯净);598 tests |
| M14 发布硬化 | `[x]` | M13 | 中 | 2(M14a/b) | R8 keep 审计(零改动,宽 keep 覆盖)+ v3.4.0 versionCode bump(896→900)+ 签名 release APK + C3 回归硬化(Generator drop-deck 权重 + MobSpawner rotation 真行为断言)+ showcase 平衡(lantern_blade Shortsword parity + shop 2× value + guide one-shot);599 tests;**v3.4.0 cut** |
| M15 main-game 集成 | `[x]` | M14 | 中 | 5(m15a/b/c/d/e) | Lua item/mob/spell 进主游戏掉落/刷怪/获取池 + shop 卖 Lua item/spell + remixed sprites 资产导入(498 PNG);`BalanceConfig.lua_*_prob` max 聚合 + mod 禁用 prob=0 守 C3(真行为断言延续 M14b);Lua 内容从 registered-but-inert 推进到主游戏真实出现 |
| M16 可玩内容包 alpha | `[x]` | M15 | 中 | 4(m16a/b/c + m16d smoke) | runtime 自定义 spriteFile(独立 PNG,`78174c2ed`)+ remixed_full minimal playable alpha(10 item/5 spell/6 mob/1 shop/1 hub,default_enabled=true,`3d36f4cfe`)+ mod diagnostics UI(ModDiagnostics+WndModDetails+status,`16baac444`)+ android smoke(assembleDebug+install+am start 无 fatal,`8632c6ed`);m16a/b/c 合 master(636 tests),m16d android smoke `[x]` 通过 |
| **合计(M0-M16)** | | | | **65 完成** | M0-M16 全完成;m16d smoke 通过(无 fatal crash);fork 从 modding beta → 可玩内容包 alpha;主游戏池注入 Lua 内容 + runtime 自定义贴图 + 开箱可玩 remixed_full + mod diagnostics UI |
...
| 2026-07-09 | **M13 全部完成**:3 feature(玩家 UX 补完 + 内置内容包)— M13a mod 卸载/更新 UI(`ModInstaller.removeMod` origin 守卫只删 EXTERNAL + WndModManager 长按卸载确认 + already_exists 文案指向卸载入口,merge `1c00cda0e`)+ M13b 自定义关卡发现 UI(`enterLevel` 门放开 registered(`isEnterAllowed` seam)+ `listEnterableLevels` + `WndCustomLevels` + WndGame「自定义关卡」按钮,merge `476658c36`)+ M13c 内置内容包 remished_lite(`default_enabled=true` showcase hub 关卡:guide/sage NPC + shop + 武器经 giveItem;main game 原版 C3 纯净 —— 不 register_trap/不改 Generator.LUA_ITEM/spawn 池不读;spell/mob registered-but-inert 标注,merge 前 9856ca98);598 tests 绿;codex infra 持续 503 → 政策 B 测试硬验收 + C3 自审;mod 系统端到端可用(卸载/更新 + 关卡发现 + 开箱内容);累计 54 feature | dispatcher |
| 2026-07-09 | **M14 全部完成 + v3.4.0 cut**:2 feature(发布硬化)— M14a R8 keep 审计 + v3.4.0 versionCode bump + 签名 release 打包(`build.gradle` 896→900 / '3.4.0';R8 宽 keep 覆盖零改动;签名 release APK 28.9MB apksigner 过;install+launch 不崩,EGL 黑屏 debug 对照证设备 quirk;merge `feab779ea`)+ M14b C3 回归硬化 + showcase 平衡(`RemishedLitePackTest` 加 Generator drop-deck `categoryProbs` 反射断言 + `MobSpawner.getMobRotation` 真行为断言;lantern_blade attackProc +2→+1 Shortsword parity + shop 统一 2× vanilla value + finite quantity + guide one-shot 守卫;保守只调 showcase-local 不动全局 BalanceConfig;merge `ff09e3c13`);599 tests 绿;codex infra 503 政策 B;**v3.4.0 发布基线确立**(versionCode 900 + git tag v3.4.0 + 签名 release APK);累计 56 feature | dispatcher |
| 2026-07-09 | **M15 全部完成**:5 feature(main-game 集成,Lua 内容从 registered-but-inert 推进到主游戏真实出现)— m15a Lua item 主游戏掉落池(`Generator.setLuaItemProbability` + `BalanceConfig.lua_item_drop_prob` max 聚合,`db1d8b2ee`)+ m15b Lua mob 主游戏刷怪池(`LuaMobFactory` + `Level.createMob` factory 分支 + `MobSpawner` prob 替换 + `lua_mob_spawn_prob`,`172e628dd`)+ m15c Lua spell 获取路径(`Generator.Category.LUA_SPELL` + `LuaSpellPool` + `lua_spell_drop_prob`,`cff423f64`)+ m15d Lua shop 卖任意 item/spell(`LuaShopItems.create` 查 LuaItemRegistry→LuaSpellRegistry→vanilla whitelist;实现 `c8a86b5c4`,PLAN 文档 commit `ce7cc5b72`)+ m15e remixed sprites 资产导入(498 PNG + SPRITE-MAP.md + MOD-SPRITES.md,`4c0607d5e`);C3 守(mod 禁用 prob=0 零注入,真行为断言延续 M14b Generator drop-deck / MobSpawner rotation);累计 61 feature | feature_worker |
| 2026-07-09 | **M16 第一批完成(m16a/b/c)+ m16d smoke 并行**:m16a runtime mod spriteFile(Lua item/spell/mob 独立 PNG + ModMobSprite 选择,628 tests,`78174c2ed`)+ m16b remixed_full minimal playable alpha(10 item / 5 spell / 6 mob / 1 shop / 1 hub,default_enabled=true,`3d36f4cfe`)+ m16c mod diagnostics UI(`ModDiagnostics` + `WndModDetails` + `WndModManager` status,636 tests,`16baac444`);m16d android smoke `[~]` 并行(`docs/SMOKE-M16.md`,状态视回报收口);fork 从 modding beta → 可玩内容包 alpha;累计 64 完成 / 65 计划 | feature_worker |
| 2026-07-10 | **M16 完成 + m16d smoke 通过**:m16d android 真机冒烟收口(`docs/SMOKE-M16.md`:`:android:assembleDebug` 33.8MB BUILD SUCCESSFUL + `adb install` Success + `am start` START_SUCCESS pid + logcat 无 FATAL/ANR,仅已知 EGL_BAD_DISPLAY 设备 quirk;UI 深度交互 SAF 导入/启用/地牢行为落为人工 checklist);**M16 全部完成(4 feature),累计 65 完成 / 0 计划**;fork 可玩内容包 alpha 确立(runtime 自定义贴图 + remixed_full 开箱可玩 + mod diagnostics UI + 真机 smoke 依据) | dispatcher |
