...
**决策:选 (a) 接受降级,combat 强度 hook 留 M7。**

理由:source-aware `Char`/`Mob` 全局 hook(`defenceProc`/`attackProc`/`drBonus`/`speedMultiplier`/`stealthBonus`/`charAct`/`regenerationBonus`/`hasteLevel`/sprite glow/source-item shield)是平台级改动,工时 >1 天,会触发"M6e 是否拆 M6e1/M6e2"评估;12 降级 buff 已保留 metadata + lifecycle 可测行为(`RpdApiBuffTest` 绿),内容包可用。`Charm`/`Terror` 未进 Java whitelist(source id 语义,M7)。

M6e 未在 M6c 侧引入新代码;M6c 的 16 buff + 6 API 在 M6e C3 全量回归中保持绿(8 registry disabled=0/enabled=full,273 tests)。
