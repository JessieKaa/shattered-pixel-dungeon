package com.shatteredpixel.shatteredpixeldungeon.modding.annotations.processor;
...
/**
 * Annotation processor for {@link LuaInterface}. Generates
 * {@code lua-interface-map.json} (written to {@link StandardLocation#CLASS_OUTPUT}
 * so it lands on the runtime classpath) describing every annotated class and the
 * public members marked safe for Lua. The transitive closure pulls in referenced
 * types that themselves carry the annotation, so an exposed method returning an
 * {@code @LuaInterface} type auto-exposes that type's annotated members too.
 *
 * <p>Logic ported from Remixed Dungeon's {@code LuaInterfaceProcessor}; package
 * adapted to the SPD fork namespace.
 */
