...
public class WndSaveSlotSelect extends Window {
	...
	private static String txt(String key, Object... args) {
		String tmpl;
	...
	public WndSaveSlotSelect(boolean saving, Runnable onCancel) {
		...
		IconTitle title = new IconTitle(
				saving ? Icons.SHPX.get() : Icons.DEPTH.get(),
				txt( saving ? "title_save" : "title_load"));
		...
		ShatteredPixelDungeon.scene().addToFront(new WndOptions(
			...
			protected void onSelect(int index) {
				if (index == 0) {
					...
					hide();
					ShatteredPixelDungeon.scene().addToFront(new WndSaveSlotSelect(saving, onCancel));
				}
			}
		});
	}
	...
	private class SlotRow extends Component {
		...
		SlotRow(final SaveSlotMeta meta) {
			...
			actionBtn = new RedButton(txt( saving ? "btn_save" : "btn_load"), 6) {
				@Override
				protected void onClick() {
					if (saving) {
		...
		protected void layout() {
			...
			actionBtn.setRect(deleteBtn.left() - MARGIN - actionW, y, actionW, height);
